...
RUN cd /opt \
    ...
    && wget -O - http://nginx.org/download/nginx-$NGINX_VERSION.tar.gz | tar zxfv - \
    && cd nginx-$NGINX_VERSION \
    && ./configure --with-compat --prefix=/opt/nginx-$NGINX_VERSION --add-dynamic-module=/opt/ngx_http_proxy_connect_module \
    && make modules
#    && ./configure \
#          --prefix=/opt/nginx-$NGINX_VERSION \
#          --add-module=/opt/headers-more-nginx-module \
...
COPY --from=builder-headers-more /opt/nginx-$NGINX_VERSION/objs/ngx_http_headers_more_filter_module.so /usr/lib/nginx/modules
COPY --from=builder-http-proxy /opt/nginx-$NGINX_VERSION/objs/ngx_http_proxy_connect_module.so /usr/lib/nginx/modules
